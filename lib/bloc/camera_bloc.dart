...
import 'package:camera_app/camerapage.dart';
...
class CameraBloc extends Bloc<CameraEvent, CameraState> {
  ...
  Future<void> _onTapFocus(TapToFocus event, Emitter<CameraState> emit) async {
    if (state is! CameraReady) return;
    final s = state as CameraReady;
    final relativeX = event.position.dx / event.previewSize.width;
    final relativeY = event.position.dy / event.previewSize.height;

    await s.controller.setFocusPoint(Offset(relativeX, relativeY));
    await s.controller.setExposurePoint(Offset(relativeX, relativeY));
  }
  ...
  Future<void> _onClearSnackBar(
    ...
    if (state is! CameraReady) return;
    final s = state as CameraReady;
    emit(s.copyWith(snackbarMessage: ''));
  }

  Future<void> _setupController(
    int index, Emitter<CameraState> emit, {
    CameraReady? previous,
  }) async {
    await previous?.controller.dispose();
    final controller = CameraController(
      _cameras[index], ResolutionPreset.max,
      enableAudio: false);
